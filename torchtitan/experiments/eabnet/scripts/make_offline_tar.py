...
import glob
'''
This is a core module that contains scripts to generate mcse sample from settings
'''
import numpy as np 
...
def cal_coeff_for_adjusting_relative_energy(wav1: np.ndarray, wav2: np.ndarray, target_dB: float):
...
def trim_silence(a, samplerate, threshold=0.01, frame_size=0.01):
    ...
    """
    assert len(a.shape) == 1, (a.shape, "multi channel audio trim is not implemented yet")
    # 计算帧能量
...
def load_and_crop(audio_path, n_sample_points, random_crop=True, rng:np.random.Generator=None):
...
def make_sample(rirs, rirs_dp, target_speech, ref_mic, noise_list, snr, dbfs, mc_noise=None):
    '''
    ...
    if rirs is not None:
        ...
        for m in np.arange(M):
            for s in np.arange(S):
                sig = sources[s]
                h = rirs[m][s]
                premix_signals[s, m] += fftconvolve(
                    h, sig
                )[:L]
        ...
        assert target_speech.shape == mc_noise.shape, (target_speech.shape, mc_noise.shape)
        target = target_speech[:,ref_mic]
        rvbt_speech = rearrange(target_speech, 'n c -> c n')
        rvbt_mixed_noise = rearrange(mc_noise, 'n c -> c n')

    scale_noise = cal_coeff_for_adjusting_relative_energy(rvbt_speech, rvbt_mixed_noise, snr)
...
def generate_surround_noise_positions(room_dim, p_mics_center, p_target, opt, rng:np.random.Generator):
    ...
    for i in range(n_noise):
        ...
        for _ in range(10):
            ...
            if (opt['min_dist_to_wall'] < x_noise < room_dim[0] - opt['min_dist_to_wall']) \
                and (opt['min_dist_to_wall'] < y_noise < room_dim[1] - opt['min_dist_to_wall']):
                break
            x_noise = None
            y_noise = None
        if x_noise != None and y_noise != None:
            p_noise_list.append((x_noise, y_noise, gen_random_config_value(opt['h'],rng)))
    return p_noise_list
...
def generate_random_rirs(opt, rng:np.random.Generator, specific=None):
    ...
    if random_target or random_mics:
        while True:
            ...
            if random_target:
                ...
                if not (d<target_x<room_dim[0]-d and d<target_y<room_dim[1]-d):
                    fail_count += 1
                    continue
    ...
    if 'noise_xyz_list' in specific:
        ...
        if noise_type == 'surround':
            ...
            while True:
                n_try += 1
                p_noise_list = generate_surround_noise_positions(room_dim, p_mics_cen, p_target, opt['noise'], rng)
...
def sample_speech(speech_list, bias, speech_root, n_sample_points, real_speech_converter, opt, rng):
    sr = opt['audio']['fs']
    speech_audio_list = []
    total_len = 0
    N_TRIES = 100
    for _ in range(N_TRIES):
        ...
        speech_path = os.path.join(speech_root,speech)
    ...
    assert total_len >= n_sample_points, f'N_TRIES={N_TRIES} may be too small? {[len(a) for a in speech_audio]}'
...
def generate_random_sample(opt,rng:np.random.Generator,speech_root,speech_list,noise_root,noise_list,clip_seconds,
                           specific=None,
    ...
    if 'speech_audio' in specific:
        ...
        speech_audio = sample_speech(speech_list, 0, speech_root, n_sample_points, real_speech_converter, opt, rng)
    ...
    random_crop = False if ('random_crop' in specific and specific['random_crop']==False) else True

    audios = [crop_audio(speech_audio, n_sample_points, random_crop=random_crop, rng=rng)] 

    mc_noise = None
    for use_noise in use_noise_list:
        if opt['noise']['type'] == 'real':
            mc_noise = real_noise_converter(load_and_crop(os.path.join(noise_root,use_noise), n_sample_points))
        else:
            audios.append(load_and_crop(os.path.join(noise_root,use_noise),n_sample_points,random_crop=random_crop,rng=rng))
    ...
    sample = make_sample(rirs, rirs_dp, audios[0], ref_mic, audios[1:], snr, dbfs, mc_noise=mc_noise)
    sample['snr'] = snr
    sample['dbfs'] = dbfs
    sample['room'] = room
    return sample
...
def read_list(list_file, data_root, data_ext='.wav'):
    if list_file is None:
        data_list = [os.path.basename(x) for x in glob.glob(os.path.join(data_root,f'*{data_ext}'))]
        return data_list

    with open(list_file,'r') as f:
        data_list = [line.strip() for line in f.readlines()]
        for x in data_list:
            assert os.path.exists(os.path.join(data_root,x)), (data_root,x)
    return data_list
  

  
def create_webdataset(opt, output_prefix, max_tar_size=5 * 1024**3):
    """
    Args:
        opt (dict): dataset options，包含 speech_root, noise_root, mcse_settings, speech_list, noise_list, clip_seconds
        output_prefix (str): 生成的 tar 文件前缀，例如 "output/dataset"
        max_tar_size (int): 每个 tar 文件的最大大小（字节），默认 5GB
    """

    speech_root = opt["speech_root"]
    noise_root = opt["noise_root"]

    with open(opt["mcse_settings"], "r") as f:
        mcse_settings = json.load(f)

    speech_list = read_list(opt["speech_list"], speech_root)
    noise_list = read_list(opt["noise_list"], noise_root)
    ...
    def _open_new_tar():
        """关闭旧 tar，新建一个 tar 文件"""
        nonlocal tar_idx, tar_path, tar
        if tar is not None:
            tar.close()
            print(f"[INFO] Closed tar: {tar_path}")
        os.makedirs(os.path.dirname(output_prefix), exist_ok=True)
        tar_path = f"{output_prefix}-{tar_idx:05d}.tar"
        tar = tarfile.open(tar_path, "w")
        tar_idx += 1
        print(f"[INFO] Opened new tar: {tar_path}")
        return tar_path, tar

    tar_path, tar = _open_new_tar()

    for i, speech_path in enumerate(tqdm(speech_list, desc="Converting to webdataset")):
        # 取一个 speech
        speech_audio = sample_speech(
            speech_list, i, speech_root, sr * clip_seconds, None, mcse_settings, rng
        )
        ...
        # print(f"Debug {type(noisy)}-{noisy.dtype}")

        key = f"{sample_idx:08d}"

        # noisy npy
        noisy_bytes = io.BytesIO()
        np.save(noisy_bytes, noisy.astype(np.float32))
        noisy_bytes.seek(0)
        noisy_info = tarfile.TarInfo(name=f"{key}.noisy.npy")
        noisy_info.size = len(noisy_bytes.getbuffer())
        tar.addfile(noisy_info, noisy_bytes)

        # clean npy
        clean_bytes = io.BytesIO()
        np.save(clean_bytes, target.astype(np.float32))
        clean_bytes.seek(0)
        clean_info = tarfile.TarInfo(name=f"{key}.clean.npy")
        clean_info.size = len(clean_bytes.getbuffer())
        tar.addfile(clean_info, clean_bytes)
    ...
    if tar is not None:
        tar.close()
        print(f"[INFO] Final tar closed: {tar_path}")
...
if __name__ == "__main__":
    ...
    "speech_root": "",
    "noise_root": "",
    "speech_list": "/home/liuxin/datasets/chinese_speech/train.txt",
    "noise_list": "/home/liuxin/datasets/single_noise/train.txt",
    "mcse_settings": "/home/liuxin/mcse/dataset/mcse_settings/v13.json",
    "clip_seconds": 6,}
    create_webdataset(opt, output_prefix="/data1/liuxin/datasets/audio_webdataset/mcse_dataset", max_tar_size=3 * 1024**3)
