...
def _save_tensor_to_tar(tar: tarfile.TarFile, key: str, tensor: torch.Tensor):
    """将 PyTorch Tensor 保存到 tar 文件"""
    tensor_bytes = io.BytesIO()
    torch.save(tensor, tensor_bytes)
    tensor_bytes.seek(0)
    tensor_info = tarfile.TarInfo(name=f"{key}.pt")
    tensor_info.size = len(tensor_bytes.getbuffer())
    tar.addfile(tensor_info, tensor_bytes)
...
def _worker(
    ...
    for i, index in enumerate(indices):
        ...
        speech_audio = sample_speech(
            speech_list, index, "", sr * clip_seconds, None, mcse_settings, rng
        )
        ...
        # 将 numpy 数组转换为 float32 的 PyTorch Tensor
        noisy_tensor = torch.tensor(noisy, dtype=torch.float32)
        target_tensor = torch.tensor(target, dtype=torch.float32)

        # === 打开/新建 tar ===
        if tar is None:
            out_path = os.path.join(out_dir, f"shard-{worker_id:03d}-{tar_idx:05d}.tar")
            tar = tarfile.open(out_path, "w")

        key = f"{worker_id:03d}_{i:08d}"
        _save_tensor_to_tar(tar, f"{key}.noisy", noisy_tensor)
        _save_tensor_to_tar(tar, f"{key}.clean", target_tensor)

        # meta = {"index": int(index), "sample_rate": int(sr)}
        # meta_bytes = io.BytesIO(json.dumps(meta).encode("utf-8"))
        # meta_info = tarfile.TarInfo(name=f"{key}.json")
        # meta_info.size = len(meta_bytes.getbuffer())
        # tar.addfile(meta_info, io.BytesIO(meta_bytes))

        # === 文件大小检查 ===
        # if tar.fileobj.tell() >= maxsize:
        #     tar.close()
        #     tar = None
        #     tar_idx += 1
        if tar_idx > 0 and tar_idx % max_samples_per_tar == 0:
            tar.close()
            tar = None
            tar_idx += 1
...
def create_webdataset_mp(
    ...
    indices = list(range(len(speech_list)))
...
if __name__ == "__main__":
    ...
    create_webdataset_mp(
        opt,
    out_dir="/data1/liuxin/datasets/audio_wds",
    maxsize=2 * 1024**3,  # 每个 tar 最大 2GB
    num_workers=16         # 开 8 个进程并行写
)
